...
class ShopProductModel extends ShopModel{
    ...
    protected $_validate = array(
        ...
        array('mrsp', '/^\d+(\.\d+)?$/', '价格只能填数字', self::VALUE_VALIDATE, 'regex', self::MODEL_BOTH),
    ...
    public function update($data){
        ...
        */
        ...
        //行为记录
        if($id){
            action_log('update_product', 'product', $id, UID);
        }
    ...
    public function checkCategory($id){
        $publish = get_shopcategory($id, 'allow_publish');
    ...
    protected function checkModel($info){
        $model = get_shopcategory($info['category_id'], 'model');
    ...
    private function listMap($category, $status = '', $pos = null){
        ...
        if(!is_null($category)){
            if(is_numeric($category)){
                ...
                $map['cid'] = array('in', str2arr($category));
            }
        }
    ...
    /*
    *获取产品图片
    *@param intger $ids 图片id
    $@return array 图片信息
    */
    public function getProductImages($ids,$limit=5)
    {
        $img_obj = M('picture');
        if(!$ids){
            $this->error = '产品图不得为空!';
        ...
        if(is_array($ids)){
            ...
            $images = $img_obj->field('id,path')->where($map)->limit($limit)->select();
        }else{
            $map = array('id'=>$ids,'status'=>'1');
            $images = $img_obj->field('id,path')->where($map)->find();
        }
        return $images;
    }
}
